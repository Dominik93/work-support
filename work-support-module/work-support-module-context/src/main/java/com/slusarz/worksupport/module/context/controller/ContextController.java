...
public class ContextController implements ContextApi {
    ...
    @Override
    public ChangeContextResponse change(@RequestBody ChangeContextRequest changeContextRequest) {
        EnvironmentTenantContext.setCurrentTenant(Environment.of(changeContextRequest.getEnvironment()));
        DatabaseTenantContext.setCurrentTenant(Database.of(changeContextRequest.getDatabase()));
        ModuleInit moduleInit = moduleInitService.getModuleInit();
        return initResponseMapper.toChangeContextResponse(moduleInit);
    }
